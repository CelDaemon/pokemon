...
class BattleTest {
    ...
    fun `Pokemon that do not have each other's weakness cause a draw`() {
        assertEquals(Battle.BattleResult.DRAW, Battle.check(Squirtle("Challenger", trainer), Squirtle("Opponent", trainer)))
    }
    @Test
    fun `Pokemon that have each other's weaknesses should cause a draw`() {
        ...
        assertEquals(Battle.BattleResult.DRAW, Battle.check(Charmander("Challenger", trainer), DrawPokemon("Opponent", trainer)))
    }

    @Test
    fun `Challenger with the same strength as the opponent's weakness wins`() {
        assertEquals(Battle.BattleResult.CHALLENGER, Battle.check(Charmander("Challenger", trainer), Bulbasaur("Opponent", trainer)))
    }

    @Test
    fun `Opponent with the same strength as the challenger's weakness wins`() {
        assertEquals(Battle.BattleResult.OPPONENT, Battle.check(Charmander("Challenger", trainer), Squirtle("Opponent", trainer)))
    }
}